    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            ae.on('mouseover', function(d, i) {
				if(d3.select(this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode).attr('id') == "FileDetailsDonutChart"){
					if(d3.select(this).attr('sliceClicked')){
						dispatch.elementMouseover({data: d.data,index: i,color: d3.select(this).style("fill")});
					}
					else{
						if (growOnHover) {
        ...
        selection.each(function(data) {
            ...
            if (showLegend) {
                ...
                if (legendPosition === "top") {
					...
					wrap.select('.nv-legendWrap').attr('availableHeight', availableHeight);
                    wrap.select('.nv-legendWrap')
                        .attr('transform', 'translate(0,' + (-margin.top) +')');
