angular.module('VolpayApp').controller('userdeleteSessionMgmtController', function($scope, $rootScope, $http, $filter, $state, userMgmtService, CommonService, $timeout, bankData, GlobalService, $location, LogoutService, $stateParams, errorservice, EntityLockService) {
    ...
    $scope.ResourcePermissionCall = function() {
        ...
        $http.post(BASEURL + RESTCALL.ResourcePermission, {
            ...
            "ResourceName": "ACH Session Management"
    ...
    var restServer = RESTCALL.SessionConnectedUsers;
    var delData = {};
    $scope.backUp = {};
    $scope.indexx = "";
    ...
    $scope.refreshCall = function() {
        ...
        restServer =  RESTCALL.SessionConnectedUsers;
        $scope.initialObj = {};
    ...
    $scope.initSetting = function() {
        ...
        {
            "label": "Usersessiondelete.Lastname",
            "value": "SurName",
            "type": "text",
            "allow": "string",
            "visible": true
        ...
        {
            "label": "Usersessiondelete.Partynames",
            "value": "EntityName",
            "type": "text",
            "allow": "string",
            "visible": true
    ...
    $scope.loadMore = function() {      
        ...
        $http.post(BASEURL +  RESTCALL.SessionConnectedUsers, $scope.uorQueryConstruct($scope.fieldArr)).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
        ...
        for (var j in Object.keys(val)) {
            if (val[Object.keys(val)[j]]) {
                if (Object.keys(val)[j] == 'Status') {
                    ...
                    $scope.fieldArr.filters.groupLvl1[0].groupLvl2[0].groupLvl3 = [{
                    ...
                    for (var i in val[Object.keys(val)[j]]) {
                        ...
                        $scope.fieldArr.filters.groupLvl1[0].groupLvl2[0].groupLvl3[0].clauses.push({
                    ...
                    $scope.fieldArr.filters.groupLvl1[0].groupLvl2[0].groupLvl3.push({
                        "logicalOperator": "AND",
                        "clauses": [{
    ...
    function crudRequest(_method,_data) {
        return $http({
            method: _method,
            url: BASEURL + RESTCALL.SessionDisConnectUsers,
            data: _data
        }).then(function(response) {
            $scope.restResponse = {
                'Status': 'success',
    ...
    $scope.checkStatusForBulk = function(event, TotalItem, allpayments1, index) {
            ...
            for (var i in TotalItem) {
                if ($('#check_' + i).prop("checked")) {
                    $scope.uniqPayments.push(TotalItem[i].UserID)
                }
            }
            $scope.createData['DisconnectUsers'] = $scope.uniqPayments
           // Object.assign($scope.createData,$scope.fieldArr)
            $.extend($scope.createData,$scope.fieldArr);
        }
        $scope.checkStatusForBulk1 = function(TotalItem) {
            if ($('.selectall').is(':checked')) {
                $('.selectall_').prop('disabled', true);
                $('.selectall_').prop('checked', false);
                $scope.createData['DisconnectUsers'] = ["selectall"]
                $.extend($scope.createData,$scope.fieldArr);
        ...
        $scope.DeleteSession = function(flag) {

               if($scope.UserAll.Selected&&flag=='true'){
                $('#delPopup').hide();
                setTimeout(function () {
                    $('.modal-backdrop').hide();
                  }, 20)
                crudRequest('POST', $scope.createData).then(function(response) {
                    $scope.alerts = [{
                ...
                }else if(!$scope.UserAll.Selected&&flag!='true'){
