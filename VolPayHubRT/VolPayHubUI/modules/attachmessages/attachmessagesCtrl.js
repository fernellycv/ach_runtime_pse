angular.module('VolpayApp').controller('attachmessagesCtrl', ['$scope', '$rootScope', '$timeout', 'PersonService', '$location', '$state', '$http', '$translate', 'GlobalService', 'bankData', '$filter', 'LogoutService', 'AttachMsgService', 'EntityLockService', function($scope, $rootScope, $timeout, PersonService, $location, $state, $http, $translate, GlobalService, bankData, $filter, LogoutService, AttachMsgService, EntityLockService) {
    $rootScope.$emit('MyEventforEditIdleTimeout', true);
    EntityLockService.flushEntityLocks(); 

    $scope.tableView = [{

            "label": "AttchMsgInstrID",
            "fieldName": "AttchMsgInstrID",
            "visible": true

        }, {
            "label": "AttchMsgID",
            "fieldName": "AttchMsgID",
            "visible": true

        }, {
            "label": "AttchMsgFunc",
            "fieldName": "AttchMsgFunc",
            "visible": true
            ...
            "fieldName": "OrigAttchMsgRef ",
            "visible": true

        },
        {
            "label": "Parent Function",
            "fieldName": "ParentMsgFunc",
            "visible": true
        },
        {
            "label": "Received Date",
            "fieldName": "TimeOfEntry",
            "visible": true
        },
        {
            "label": "Distribution Reference",
            "fieldName": "AttchMsgDistrID",
            "visible": true
        }
    ]
    ...
    $scope.retainSavedSearch = function() {
        ...
        $http.post(BASEURL + RESTCALL.userProfileData + '/readall', $scope.uDetails).then(function onSuccess(response) {
            ...
            if (Object.keys($scope.uData).indexOf('savedSearch') != -1) {
                if ("AllAttachedMessages" in $scope.uData.savedSearch) {
                    if ($scope.uData.savedSearch.AllAttachedMessages.length) {
                        for (var i in $scope.uData.savedSearch.AllAttachedMessages) {
                            $scope.lskey.push($scope.uData.savedSearch.AllAttachedMessages[i].name)
                        }
                    } else {
                        $scope.uData.savedSearch.AllAttachedMessages = [];
                ...
                userData.savedSearch.AllAttachedMessages = [];
    ...
    $scope.commonObj = AttachMsgService.distInstruction;

    AttachMsgService.distInstruction.currentObj.sortBy = [];
    ...
    $scope.dateSet = function() {
        ...
        $scope.dateFilter = AttachMsgService.distInstruction.dateFilter;
    ...
    $scope.retExpResult = function() {
        ...
        if ($scope.dateFilter.all) {
            ...
            $scope.customDate.startDate = AttachMsgService.distInstruction.customDate.startDate;
            $scope.customDate.endDate = AttachMsgService.distInstruction.customDate.endDate;
    ...
    $scope.uorQueryConstruct = function(arr, flag) {
        ...
        AttachMsgService.distInstruction.currentObj = arr;
        $scope.fieldArr = arr;
    ...
    $scope.initCall = function(_query) {
        ...
        sessionStorage.FileListCurrentRESTCALL = BASEURL + RESTCALL.AttachMessages;
        ...
        $http.post(BASEURL + RESTCALL.AttachMessages, _query).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
    ...
    $scope.refresh = function() {
        ...
        AttachMsgService.distInstruction = {
        	currentObj: {
        		"sortBy": [],
        		"params": [],
        		"start": 0,
    ...
    $scope.FilterByDate = function(params, eve) {
        ...
        if ($scope.dateFilter.custom) {
            ...
            $('#customDate').modal('hide')
            AttachMsgService.distInstruction.customDate.startDate = $scope.customDate.startDate;
            AttachMsgService.distInstruction.customDate.endDate = $scope.customDate.endDate;
        }

        $scope.dateArr = $scope.retExpResult()
        for (var i in $scope.fieldArr.params) {
            if (($scope.fieldArr.params[i].ColumnName == 'TimeOfEntry') || (!$scope.fieldArr.params[i].advancedSearch)) {
    ...
    $scope.$watch('fieldArr.params', function(nArr, oArr) {
        ...
        if (nArr.length) {
            AttachMsgService.distInstruction.searchFound = true;
        } else {
            AttachMsgService.distInstruction.searchFound = false;
        }

        //$scope.searchFound = AttachMsgService.distInstruction.searchFound;
    })
    ...
    $scope.uorSearch = function() {
        ...
        if ($scope.uorVal) {
            ...
            AttachMsgService.distInstruction.uorVal = $scope.uorVal;
            len = 20;
            $scope.uorSearchFound = false;

            for (var i in $scope.fieldArr.params) {
                if (!$scope.fieldArr.params[i].advancedSearch) {
                    if ($scope.fieldArr.params[i].ColumnName == 'AttchMsgInstrID') {
                        $scope.uorSearchFound = true;
                        $scope.fieldArr.params[i].ColumnName = 'AttchMsgInstrID';
    ...
    $scope.getExistVal = function(eve) {
        ...
        if ($scope.uorVal) {
            ...
            if ($scope.dateFilter.all) {
                $scope.uorFound = '';
                AttachMsgService.distInstruction.uorVal = '';
    ...
    $scope.clearSearch = function() {
        ...
        //$scope.resetFilter();
        AttachMsgService.distInstruction.dateFilter = {
            all: true,
            today: false,
            week: false,
            month: false,
    ...
    $scope.loadMore = function(query) {
        ...
        $http.post(BASEURL + RESTCALL.AttachMessages, $scope.uorQueryConstruct($scope.fieldArr)).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
    ...
    $scope.buildSearch = function() {
        ...
        return {
            'service': AttachMsgService.distInstruction,
    ...
    $scope.saveSearch = function() {
        ...
        if ($scope.searchname) {
            ...
            for (var key in userData.savedSearch.AllAttachedMessages) {
                if (userData.savedSearch.AllAttachedMessages[key].name == $scope.searchname) {
        ...
        if (!$scope.searchNameDuplicated) {
            ...
            if ($scope.searchname) {
                ...
                if ($scope.searchSet) {
                    ...
                    if ('AllAttachedMessages' in userData.savedSearch) {
                        userData.savedSearch['AllAttachedMessages'].push({
    ...
    $scope.saveSearch1 = function() {
        ...
        userData.savedSearch.AllAttachedMessages[$scope.keyIndex].name = $scope.searchname;
        userData.savedSearch.AllAttachedMessages[$scope.keyIndex].params = saveSearchObjects;
        for (var i in userData.defaultChartTypes.paymentDashoard) {
    ...
    $scope.selectSearch = function(data, eve, index) {
        ...
        if ($(eve.currentTarget).find('span:first-child').text() != 'New Search') {
            ...
            $scope.search = $scope.uData.savedSearch.AllAttachedMessages[index - 1].params.searchParams;
    ...
    $scope.clickReferenceID = function(val) {
        ...
        /*	GlobalService.fileListId = val.data.AttchMsgInstrID;
			GlobalService.UniqueRefID = val.data.AttchMsgID;
			...
			$scope.Obj = {
				'uor': '',
				'info': val.data.AttchMsgFunc,
				'nav': {
					'UIR': val.data.AttchMsgInstrID,
					'PID': val.data.AttchMsgID
				},
				'from': 'allpayments'
			}
    ...
    $scope.makeCall = function() {
        ...
        $http.post(BASEURL + RESTCALL.AttachMessages, $scope.makeObj).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
            ...
            var colName = ["AttchMsgInstrID", "AttchMsgID", "AttchMsgFunc", "ParentID", "PSACode", "OrigAttchMsgRef", "ParentMsgFunc", "TimeOfEntry", "AttchMsgDistrID"];
            JSONToExport(bankData, $scope.items, 'AllAttachedMessages', true, colName);
        }).catch(function onError(response) {
            // Handle error
            var data = response.data;
            var status = response.status;
    ...
    $scope.exportToExcel = function() {
        if ($("input[name=excelVal][value='All']").prop("checked")) {
            ...
            JSONToExport(bankData, $scope.dat, 'AllAttachedMessages', true, colName);
        }
    ...
    $scope.deleteSelectedSearch = function(eve) {
        ...
        userData.savedSearch.AllAttachedMessages.splice($scope.selectedSearchName - 1, 1)
        for (var i in userData.defaultChartTypes.paymentDashoard) {
            if (userData.defaultChartTypes.paymentDashoard[i].data) {
                delete userData.defaultChartTypes.paymentDashoard[i].data;
            }
