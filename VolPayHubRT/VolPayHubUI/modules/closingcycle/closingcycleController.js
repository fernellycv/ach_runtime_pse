angular.module('VolpayApp').controller('closingcycleCtrl', function ($scope, $rootScope, $timeout, PersonService, $location, $state, $http, $translate, GlobalService, bankData, $filter, LogoutService, CommonService, RefService, errorservice, EntityLockService, GetPermissions) {

    $scope.newPermission = GetPermissions("Closing Cycle");
    ...
    $scope.setSortMenu = function () {
        $scope.sortMenu = [
            ...
            {
                "label": "CyclesModule.ProcessDate",
            ...
            // {
            //     "label": "CyclesModule.CreatedDateTime",
            ...
            {
                "label": "securityRolesPermissions.Action",
                "fieldName": "Action",
                "visible": true,
                "searchVisible": false,
                "type": "text"
    ...
    $scope.initCall = function (_query) {
        ...
        $http.post(BASEURL + RESTCALL.GetAllCycle, _query).then(function onSuccess(response) {
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
            var headers = response.headers;
    ...
    $scope.FilterByDate = function (ProcessDate) {
        ...
        $scope.reqBodyObj = { ProcessDate: ProcessDate };
    ...
    $scope.refresh = function (booleanVal) {
        $scope.ProcessDate = moment(new Date()).format('YYYY-MM-DD')
        $scope.reqBodyObj = { ProcessDate: $scope.ProcessDate };
        $scope.reqBodyObj.start = 0;
        $scope.reqBodyObj.count = 20
    ...
    $scope.triggerCondition = function (arg) {
        ...
        $http.post(BASEURL + '/rest/v2/cycle/cycleclosingresponse', $rootScope.Totalobj).then(function onSuccess(response) {
            // Handle success
            $('#myModal').hide();
            $('.modal-backdrop ').hide();
            var data = response.data;
            ...
            $scope.alerts = [{
                type: 'success',
                msg: data.successResponse.message
            }];
            ...
            if ((data.errorResponse.statusCode == 400) || (data.errorResponse.statusCode == 401)) {
                $('#myModals').modal('show')
                $scope.errormsg = data.errorResponse.message
            } else {
                // $('#myModals').modal('hide')
    ...
    $scope.forceActionpopup = function (arg) {
        $rootScope.Totalobj = arg
    }
    ...
    $scope.detail = function (val, index) {
        ...
        $http.post(BASEURL + '/rest/v2/cycle/audit/read', val).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
    ...
    $scope.loadMore = function (query) {
        ...
        $scope.fieldArr.ProcessDate = $scope.ProcessDate
        $http.post(BASEURL + RESTCALL.GetAllCycle, $scope.uorQueryConstruct($scope.fieldArr)).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
