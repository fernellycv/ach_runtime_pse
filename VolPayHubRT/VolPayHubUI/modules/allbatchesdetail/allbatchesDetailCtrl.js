angular.module('VolpayApp').controller('allBatchesDetailCtrl', function($scope, $http, $location, $state, $timeout, $filter, $rootScope, GlobalService, AllBatchesGlobalData, RefService, bankData, errorservice) {
    ...
    if ($state.params.input == '') {
        $state.go('app.allbatches')
    } else {

        $scope.uor = $state.params.input.uor;

        $scope.UniqueRefID = $state.params.input.nav.BID;
        $scope.attachMgsFunc = $state.params.input.nav.AttachMsg;
        $scope.BatchID = $state.params.input.nav.BatchID;
        $scope.BatchType = $state.params.input.nav.BatchType;
        $scope.attachparentID = $state.params.input.parentID;
        $scope.attachrootBatchID = $state.params.input.rootBatchID;
        ...
        $scope.isBatchDetailsCollapsed = false;
        $scope.isBatchInfoCollapsed = true;
        $scope.isBatchPayInfoCollapsed = true;
        $scope.isBatchColInfoCollapsed = true;
        $scope.isBatchProInfoCollapsed = true;

        $scope.isBatchPayTypeInfoCollapsed = true;
        $scope.isBatchDebtorCollapsed = true;
        $scope.isBatchDebtorAccCollapsed = true;
        $scope.isBatchDebtorAgeCollapsed = true;
        $scope.isBatchDebtorAgeAccCollapsed = true;
        $scope.isBatchUltimateDebtorCollapsed = true;
        $scope.isBatchChargesAccCollapsed = true;
        $scope.isBatchChargesAccAgeCollapsed = true;

        $scope.isBatchColPayTypeInfoCollapsed = true;
        $scope.isBatchCreditorCollapsed = true;
        $scope.isBatchCreditorAccCollapsed = true;
        $scope.isBatchCreditorAgeCollapsed = true;
        $scope.isBatchCreditorAgeAccCollapsed = true;
        $scope.isBatchUltimateCreditorCollapsed = true;
        $scope.isBatchCreditorChargesAccCollapsed = true;
        $scope.isBatchCreditorChargesAccAgeCollapsed = true;
        $scope.isCreditorSchemaIdentificationCollapsed = true;

        $scope.isBatchPayTypeInfoSLCollapsed = true;
        $scope.isBatchLocalInstrumentCollapsed = true;
        $scope.isBatchCatPurCollapsed = true;
        $scope.isBatchReqExePurCollapsed = true;

        $scope.isDebtorPostalAddressCollapsed = true;
        $scope.isIndentificationCollapsed = true;
        $scope.isDateandPlaceofBirthCollapsed = true;
        $scope.isDebtorContactDetailsCollapsed = true;

        $scope.isUltimateDebtorPostalAddressCollapsed = true;
        $scope.isUltimateIndentificationCollapsed = true;
        $scope.isUltimateDateandPlaceofBirthCollapsed = true;
        $scope.isUltimateDebtorContactDetailsCollapsed = true;

        $scope.isUltimateCreditorPostalAddressCollapsed = true;
        $scope.isUltimateCreditorIndentCollapsed = true;
        $scope.isUltimateCreditorDateandPlaceofBirthCollapsed = true;
        $scope.isUltimateCreditorContactDetailsCollapsed = true;

        $scope.isDebtorAgePostalAddressCollapsed = true;
        $scope.isDebtorAgeIndentCollapsed = true;
        $scope.isDebtorAgeBranchDetailsCollapsed = true;
        $scope.isDebtorAgBranchPosAddCollapsed = true;

        $scope.isChargeAccAgePostalAddressCollapsed = true;
        $scope.isChargeAccAgeIndentCollapsed = true;
        $scope.isChargeAccAgeBranchDetailsCollapsed = true;
        $scope.isChargeAccAgeBranchPosAddCollapsed = true;

        $scope.isCredChargeAccAgePostalAddressCollapsed = true;
        $scope.isCredChargeAccAgeIndentCollapsed = true;
        $scope.isCredChargeAccAgeBranchDetailsCollapsed = true;
        $scope.isCredChargeAccAgeBranchPosAddCollapsed = true;

        $scope.isCSIChargeAccAgePostalAddressCollapsed = true;
        $scope.isCSIChargeAccAgeIndentCollapsed = true;
        $scope.isCSIChargeAccAgeDateandPlaceofBirthCollapsed = true;
        $scope.isCSIChargeAccAgeContactDetailsCollapsed = true;

        $scope.isBatchCollPayTypeInfoSLCollapsed = true;
        $scope.isBatchCollLocalInstrumentCollapsed = true;
        $scope.isBatchCollCatPurCollapsed = true;

        $scope.isCreditorPostalAddressCollapsed = true;
        $scope.isCreditorIndentificationCollapsed = true;
        $scope.isCreditorDateandPlaceofBirthCollapsed = true;
        $scope.isCreditorContactDetailsCollapsed = true;

        $scope.isCreditorAgePostalAddressCollapsed = true;
        $scope.isCreditorAgeIndentCollapsed = true;
        $scope.isCreditorAgeBranchDetailsCollapsed = true;
        $scope.isCreditorAgeBranchPosAddCollapsed = true;

        $scope.isInstrInfoCollapsed = true;
        $scope.morefileInfoCollapsed = false;

        $scope.loading = false;
        $scope.AuditTableLoaded = false;
        $scope.TransInfoTableLoaded = false;
        ...
        function fillData() {
            ...
            for (var i = 0; i < $scope.dropdownVal.length; i++) {
                if ($scope.dropdownVal[i]["BatchID"] == $scope.UniqueRefID) {
            ...
            $scope.batchObj.BatchID = $scope.UniqueRefID;
            var aa, attachedRestcalls;

            if ($scope.batchObj.BatchID == $scope.BatchID) {
                attachedRestcalls = BASEURL + '/rest/v2/batches/read/' + $scope.batchObj.BatchID;
            } else if ($scope.batchObj.BatchID != $scope.BatchID) {
                if ($scope.attachrootBatchID) {
                    attachedRestcalls = BASEURL + '/rest/v2/batches/read/' + $scope.batchObj.BatchID;
                }
            }

            if ($scope.instructionType || $scope.attachMgsFunc || $scope.AttachFunc) {
                aa = ($scope.instructionType == 'RESPONSE') ? (BASEURL + '/rest/v2/payments/readpcdforconf/' + $scope.batchObj.BatchID) : (($scope.attachMgsFunc != "Customer Credit Transfer" && $scope.attachMgsFunc != "Request for Payment" && $scope.attachMgsFunc != "Payment Instruction" && $scope.attachMgsFunc != "RESPONSE" && !$scope.AttachFunc) ? (attachedRestcalls) : (($scope.AttachFunc) ? (BASEURL + '/rest/v2/payments/attachmsg/' + $scope.batchObj.BatchID) : (BASEURL + RESTCALL.AllBatchesListSpecificREST)));
            } else {
                aa = BASEURL + RESTCALL.AllBatchesListSpecificREST;
            }
            ...
            $http({
                ...
                if ($scope.cData && $scope.cData['BatchStatus'] && $scope.cData['BatchStatus'].toLowerCase() === 'accepted') {
                    $scope.checkIfAttachMsgAccepted($scope.cData);
                }
                $scope.refId = $scope.cData.InstructionID;
                $scope.UniqueRefID = $scope.cData.BatchID;
                $scope.MOPForConfirm = $scope.cData.MethodOfPayment;

                $scope.showOverride = false;
                $scope.statusCheck = $scope.cData.BatchStatus.split('_');
                ...
                if ($scope.cData && $scope.cData.DetailedStatusInfo && $scope.cData.DetailedStatusInfo.indexOf('Duplicate') != -1) {
                    ...
                    if (str1.indexOf(',') != -1) {
                        $scope.duplicateBatchIDs = str1.split(', ');
                    } else {
                        $scope.duplicateBatchIDs = [str1];
                    }
                }
                ...
                $scope.statusREST = {
                    "Queryfield": [{
                        ...
                        "ColumnOperation": "=",
                        "ColumnValue": $scope.cData.BatchStatus
                    }]
                }
                ...
                $http.post(BASEURL + RESTCALL.BatchRelatedMsgREST, {
                        'BatchID': $scope.UniqueRefID
                    ...
                    // var batchPaymentInfo;
                    // if ($scope.BatchType.toLowerCase() == "processing") {
                    //     batchPaymentInfo = RESTCALL.ProcessingBatchTranInfoREST
                    // } else if ($scope.BatchType.toLowerCase() == "output") {
                    //     batchPaymentInfo = RESTCALL.OutputBatchTranInfoREST
                    // } else {
                    //     batchPaymentInfo = RESTCALL.InputBatchTranInfoREST
                    // }

                $http.post(BASEURL + RESTCALL.InputBatchTranInfoREST, {
                    'BatchID': $scope.UniqueRefID
                }).then(function(paymentAudit) {

                    $scope.transInfoAudit = paymentAudit.data;
                    $scope.TransInfoTableLoaded = true;
                }, function(err) {
                    errorservice.ErrorMsgFunction(err, $scope, $http, err.status)
                })

                $http.post(BASEURL + RESTCALL.BatchAuditREST, {
                    'BatchID': $scope.UniqueRefID
                }).then(function(paymentAudit) {
                ...
                $http.post(BASEURL + RESTCALL.BatchBIDREST, {
                    'BatchID': $scope.UniqueRefID
                ...
                $http.post(BASEURL + RESTCALL.BatchAccountPostingsInfo, {
                    'BatchID': $scope.UniqueRefID
                ...
                $http.post(BASEURL + '/rest/v2/batches/errorinformation/readall', {
                    'BatchID': $scope.UniqueRefID
                }).then(function onSuccess(response) {
                    // Handle success
                    var data = response.data;
        ...
        $scope.fetchDataAgain = function() {
            $scope.UniqueRefID = $scope.batchObj.BatchID;
            fillData()
        }
        ...
        $scope.addNotes = function(notes, toDetails) {
            $scope.Notes = {
                ...
                "BatchID": toDetails.BatchID,
                "Notes": notes.notes
            }
            $http.post(BASEURL + '/rest/v2/batches/notes', $scope.Notes).then(function(notes) {
                $scope.alerts = [{
                    type: 'success',
                ...
                $http.post(BASEURL + RESTCALL.BatchAuditREST, $scope.batchObj).then(function(paymentAudit) {
        ...
        if ($scope.fromPage == "filedetail" || $scope.fromPage == "AllConfirmaion") {
            ...
            $scope.dropdownVal = [];
            $scope.dropdownVal = (AllBatchesGlobalData.allPaymentDetails) ? AllBatchesGlobalData.allPaymentDetails : [];
            fillData();
        }
        ...
        $scope.NrP = function(val) {
            ...
            $scope.BatchID = $scope.UniqueRefID;
        ...
        function getForceAction() {
            ...
            $http.post(BASEURL + RESTCALL.ActionREST, {
                "ProcessStatus": $scope.cData.BatchStatus,
                "WorkFlowCode": "BATCH",
                "PartyServiceAssociationCode": $scope.cData.InputReferenceCode,
                ...
                if (response.data.length > 0) {
                    ...
                    $scope.PayId = $scope.cData.BatchID;
        ...
        $scope.resendAction = function(values) {
            ...
            $http({
                ...
                $http.post(BASEURL + RESTCALL.BatchBIDREST, $scope.batchObj).then(function(paymentBID) {
                    $scope.cDataBID = paymentBID.data;
                    $scope.BIDLoaded = true;
                }, function(err) {})
            }).catch(function onError(response) {
        ...
        $scope.clickReference = function(val) {
        ...
        $scope.goToPaymentSummary = function(val) {
            ...
            $scope.Obj = {
                ...
                'nav': {
                    'UIR': $scope.refId,
                    'BID': $scope.UniqueRefID
                },
                'from': $state.params.input.from
                    //'from': ($scope.fromPage == 'allpabatches')?'allbatches':'filedetail'
            }
        ...
        $scope.gotoPaymentDetail = function() {
            ...
            GlobalService.UniqueRefID = $state.params.input.BID;
        ...
        $scope.checkCondition = function(argu, payment) {
            ...
            for (var stat in statusArr) {
                if (statusArr[stat] === argu['BatchStatus']) {
                    Isstatus = true;
                    break;
                }
            }

            if (argu['BatchStatus'].indexOf('TIMEDOUT') != -1) {
                Isstatus = true;
            }
            var IsSynchronous = !argu['IsSynchronous'];
            var IsrelationShipStatus = (argu['Relationship'] === 'REQUEST') ? true : false;
            /*if(payment['BatchStatus'] && (payment['BatchStatus'].toLowerCase() === 'accepted')){
            ...
            if (Isstatus && IsSynchronous && IsrelationShipStatus) {
                if ($scope._AttachMsgAccepted.length && argu['InvocationPoint'] === 'FRAUD_ATTCHMSG_TXN') {
                    styleObj = true;
                } else if (payment['BatchStatus'].toLowerCase() !== 'fraud_hold' && payment['BatchStatus'].toLowerCase().indexOf(argu['InvocationPoint'].toLowerCase()) != -1) {
                    styleObj = true;
                }
            }
            return styleObj;
        ...
        $http.get(BASEURL + '/rest/v2/batches/taskstatus/' + $scope.UniqueRefID).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
        ...
        $scope.gotoOriginalPaymentRecord = function(batchid) {
            $scope.Obj = {
                'nav': {
                    'BID': batchid
                },
                'from': 'allbatchesdetail'
            }
        ...
        $scope.ResendOUTPayments = function(argudata) {
            ...
            $http.post(BASEURL + '/rest/v2/transports/resend/' + argudata.GroupInteractionUniqueID).then(function onSuccess(response) {
                // Handle success
                var data = response.data;
                var status = response.status;
                var statusText = response.statusText;
