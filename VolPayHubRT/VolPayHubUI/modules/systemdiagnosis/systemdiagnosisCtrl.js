angular.module('VolpayApp').controller('systemdiagnosisCtrl', function($scope, $state, bankData, $filter, GlobalService, $timeout, $location, $http, LogoutService, errorservice,datepickerFaIcons) {
    ...
    $scope.sortMenu = [{
        "label": "SystemDiagnosisDet.TableReportID",
        "FieldName": "ReportID",
        "visible": true,
        "Type": "String"
    }, {
        "label": "SystemDiagnosisDet.TableGeneratedOn",
        "FieldName": "GeneratedOn",
        "visible": true,
        "Type": "String"
    }, {
        "label": "SystemDiagnosisDet.TableUserID",
        "FieldName": "UserID",
        "visible": true,
        "Type": "String"
    }]
    ...
    var restServer = RESTCALL.SystemDiagnosisRead;
    var delData = {};
    $scope.backUp = {};
    $scope.indexx = "";
    ...
    $scope.createData = function(newData) {
        restServer = RESTCALL.SystemDiagnosisRead;
    ...
    $scope.deleteData = function() {
        ...
        restServer = RESTCALL.SystemDiagnosisRead + 'delete';
    ...
    $scope.searchFilter = function(_val) {
        ...
        for (var j in Object.keys(_val)) {
            if (_val[Object.keys(_val)[j]]) {
                ...
                } else if (Object.keys(_val)[j] == 'GeneratedOn') {
                    $scope.bankData.filters.groupLvl1[0].groupLvl2[0].groupLvl3.push({
                        "logicalOperator": "AND",
                        "clauses": [{
                                "columnName": Object.keys(_val)[j],
    ...
    function getCountbyStatus(argu) {
        crudRequest("GET", RESTCALL.SystemDiagnosisRead + argu.actualvalue + '/count', "").then(function(response) {
    ...
    $scope.ExportMore = function(argu, excelLimit) {
        if (argu > excelLimit) {
            JSONToCSVConvertor($scope.dat, (argu > excelLimit) ? 'System Diagnosis_' + ('' + excelLimit)[0] : 'System Diagnosis', true);
            $scope.dat = [];
            excelLimit += 100000
        }

        crudRequest("POST", restServer, {
            "start": argu,
            "count": ($scope.totalForCountBar > 1000) ? 1000 : $scope.totalForCountBar
    ...
    $scope.exportAsExcel = function() {
        ...
        if ($("input[name=excelVal][value='All']").prop("checked")) {
            ...
            JSONToCSVConvertor($scope.dat, 'System Diagnosis', true);
        }
    }
    ...
    $scope.gotoViewSystemDiag = function(data) {
        if (data) {
            $state.go('app.viewsystemdiagnosis', {
                input: data
            })
        }
    }

    $scope.runentityheathChck = function() {
        var genReport = false;
        crudRequest('GET', RESTCALL.RunSystemDiagnosis + '/' + genReport).then(function() {
            $scope.alerts = [{
                'type': 'success',
                'msg': 'Entity Health Check Completed Successfully'
            }]
            $scope.loadData();
        ...
        setTimeout(function() {
            ...
            // bankData.crudRequest("GET", RESTCALL.RunSystemDiagnosis + '/' + genReport, '').then(applyRestData, errorFunc);
    }
