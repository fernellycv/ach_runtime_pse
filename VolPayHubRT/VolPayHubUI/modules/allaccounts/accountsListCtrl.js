angular.module('VolpayApp').controller('accountsListCtrl', function($scope, $http, $location, $state, $timeout, $filter, $translate, $rootScope, GlobalService, AllPaymentsGlobalData, AllAccountService, bankData, errorservice, PersonService, EntityLockService) {
    ...
    if ($rootScope.PaymentIntiated && 'responseMessage' in $rootScope.PaymentIntiated) {
        ...
        setTimeout(function() {
            sessionStorage.menuSelection = JSON.stringify({ "val": "AccountsModule", "subVal": "ReceivedAccounts" })
            sidebarMenuControl('AccountsModule', 'ReceivedAccounts');
        }, 500)
            ...
            if (items.response == 'Error') {
                ...
                if (items.data.error.code == 401) {
                    if (configData.Authorization == 'External') {
                        window.location.href = '/VolPayUI' + configData['401ErrorUrl'];
                    } else {
    ...
    $scope.clickRefId = function(id, index) {
        ...
        $location.path('app/receivedaccountslist')
    }
    ...
    $scope.selectSearch = function(index) {
        ...
        if ($scope.searchName != 'New Search') {
            ...
            setTimeout(function() {
                ...
                $(sanitize("select[name="+$scope.dynamicArr[i]+"]")).select2()
                }*/
    ...
    $scope.makeCall = function() {
        ...
        $http.post(BASEURL + RESTCALL.AllFileListREST, $scope.details).then(function onSuccess(response) {
            // Handle success
            var data = response.data;
            var status = response.status;
            var statusText = response.statusText;
